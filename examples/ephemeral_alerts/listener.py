"""
Ephemeral Alert — Listener Mode
=================================

Subscribes to alert events published by the owner instance
without running the evaluator. Includes an interactive menu
for acknowledging and muting alerts.

Usage:
    python examples/ephemeral_alerts/listener.py

Requires:
    - RELAY_API_KEY environment variable
    - RELAY_SECRET environment variable
    - owner.py must be running first
"""
...
ALERT_NAME = 'ephemeral_multi_metric'
...
async def main():
    ...
    print('Listening for alert events (listener mode).')
    print_menu()
        ...
        try:
            ...
            if cmd == '1':
                ...
                result = await app.alert.ack({
                    ...
                    'ack_notes': 'Acked from listener terminal',
                })
                ...
                result = await app.alert.ack_all({
                    'alert_id': alert_id,
                    'acked_by': 'listener_operator',
                    'ack_notes': 'AckAll from listener terminal',
                })
                ...
                mute_till = (datetime.now(timezone.utc) + timedelta(seconds=60)).strftime(
                    '%Y-%m-%dT%H:%M:%S.'
